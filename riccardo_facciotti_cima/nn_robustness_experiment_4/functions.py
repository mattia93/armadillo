...
def plot_attack(true_image, attacked_image, model, function):
    ...
    a = tf.reshape(true_image, shape=[-1, 28, 28, 1])
    b = tf.reshape(attacked_image, shape=[-1, 28, 28, 1])
    #true_predictions = model.predict(np.expand_dims(a, axis=0))
    true_predictions = model.predict(a)
    #attacked_predictions = model.predict(np.expand_dims(b, axis=0))
    attacked_predictions = model.predict(b)
    # true bars
